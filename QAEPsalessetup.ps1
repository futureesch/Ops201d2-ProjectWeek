...
# The channel Craft Computing provided a nice visual-learner's walkthrough for getting a pre-build from an ISO into a WIM for WDS
# https://www.youtube.com/watch?v=PdKMiFKGQuc
# https://www.youtube.com/redirect?event=video_description&redir_token=QUFFLUhqa21vOTBLcm1XcWJBOXdMS2cwZjItR3JLdTBSd3xBQ3Jtc0trbHo2dFZXbm4yblAzZWY0dS1FVG9yckE1TElwbGFqYmxEU3l1R0FfbGdFM0s2U3lHZlJ4ZkdMeWpwVHRvYV9wUHowcFE1VUlXa0FxNXNmX0ppN29tZmRCdlg3Vko2dFh6M0VqT2lBS3ppd3dtMkNVdw&q=https%3A%2F%2Fdrive.google.com%2Ffile%2Fd%2F1p5kzaeLoBzUDKqH1p4cgvi2sNShFHU5i
# Below are all taken from the 2020-DeCrapify.ps1
# Each script has a reverse, should the client wish to change features back.

# START QAEP client Requests

# --Removing bloatware

# Without this, much of what we subsequently try to run, may not complete, and we have a lot to do.
...
# Disable Telemetry - Microsoft has a lot of built-in tracking software that requires manipulation of the Registry
Write-Host "Disabling Telemetry..."
Set-ItemProperty -Path "HKLM:\Software\Policies\Microsoft\Windows\DataCollection" -Name "AllowTelemetry" -Type DWord -Value 0

# Disable Wi-Fi Sense - We will not be using Wi-Fi
...
# Disable SmartScreen Filter - We will rely on Windows Defender, Acceptable Use Policy, and SOPs.
Write-Host "Disabling SmartScreen Filter..."
Set-ItemProperty -Path "HKLM:\Software\Microsoft\Windows\CurrentVersion\Explorer" -Name "SmartScreenEnabled" -Type String -Value "Off"
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\AppHost" -Name "EnableWebContentEvaluation" -Type DWord -Value 0

# Disable Bing Search in Start Menu - Sales persons need to be able to search for LOCAL files here, not the Internet - Brave + DuckduckGo for Privacy additions
Write-Host "Disabling Bing Search in Start Menu..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Search" -Name "BingSearchEnabled" -Type DWord -Value 0

# Disable Location Tracking - Privacy setting 
Write-Host "Disabling Location Tracking..."
Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Sensor\Overrides\{BFA794E4-F964-4FDB-90F6-51056BFE4B44}" -Name "SensorPermissionState" -Type DWord -Value 0
Set-ItemProperty -Path "HKLM:\System\CurrentControlSet\Services\lfsvc\Service\Configuration" -Name "Status" -Type DWord -Value 0

# Disable Feedback - Users do not need to be bothered with OS feedback on our deployment, since we're the help, not Windows
...
# Disable Advertising ID - Distraction for a Sales person... imagine random business reps bursting in the middle of a kitchen to announce something
...
# Disable Cortana - Literally listens to you and Microsoft does not need your sales conversations recorded and for free!
...
# Restrict Windows Update P2P only to local network - maintains workflow for reps to not have computers restart at Windows behest
...
# Remove AutoLogger file and restrict directory - Again, company activity and data should be private.
...
# Stop and disable Diagnostics Tracking Service - More invasive tracking we remove
...
# Disable Windows Update automatic restart - removes a potentially disruptive decision at Windows behest
Write-Host "Disabling Windows Update automatic restart..."
Set-ItemProperty -Path "HKLM:\Software\Microsoft\WindowsUpdate\UX\Settings" -Name "UxOption" -Type DWord -Value 1

Write-Host "Showing all tray icons..." - visual feedback to facilitate troubleshooting support
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer" -Name "EnableAutoTray" -Type DWord -Value 0

# Show known file extensions - important for everyone to understand file extensions to know what kind of data they are handling
Write-Host "Showing known file extensions..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "HideFileExt" -Type DWord -Value 0

# Change default Explorer view to "Computer" - Takes users to This PC or QumuloSalesPC easing user access to client data
Write-Host "Changing default Explorer view to `"Computer`"..."
Set-ItemProperty -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced" -Name "LaunchTo" -Type DWord -Value 1

# Disable OneDrive - we are implementing a host File Server solution
...
# Uninstall OneDrive (WINDOWS WILL NOT SYSPREP WITHOUT IT!) - Craft Computing Contribution
...
# Get-AppxPackage "Microsoft.WindowsAlarms" | Remove-AppxPackage - For time-managment without using a phone
# Get-AppxPackage "Microsoft.WindowsCamera" | Remove-AppxPackage - To allow for customer service
...
# Uninstall Windows Media Player - Based on client specs
Write-Host "Uninstalling Windows Media Player..."
dism /online /Disable-Feature /FeatureName:MediaPlayback /Quiet /NoRestart

# Uninstall Work Folders Client - reduce potential confusion over backup
Write-Host "Uninstalling Work Folders Client..."
dism /online /Disable-Feature /FeatureName:WorkFolders-Client /Quiet /NoRestart

# Set Photo Viewer as default for bmp, gif, jpg and png - provides faster workflow
...
# Show Photo Viewer in "Open with..." - closing the loop...
...
# I've selected out features a Sales Rep woudln't need; but leaves helpful user and admin services
...
# seeding updates to other computers. - These are endpoint terminals for Sales Reps; let the machines run!
#
...
# Per client request, I have removed distracting software
...
# Thank you Matthew Israelsson - Prior attribution, but sure, thanks, Matthew!
...
# This script removes all Start Menu Tiles from the .default user # - Tested and approved for client 
...
# Prevents SYSPREP from freezing at "Getting Ready" on first boot                          # - Craft Computing Contribution from here on out
...
# Set Windows to Dark Mode # - Part of the source script, kept in for energy efficiency and eye-ball purposes
